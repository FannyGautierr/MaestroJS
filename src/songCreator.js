...
export default class SongCreator {
    ...
    createDistortion(audioContext){
        ...
        return distortion;
            ...
            throw new TypeError('Failed to create distortion:', error.message)
        }
    }
    /**
     ...
     */
      addTimedTrack(startTime, endTime) {
    ...
    stopTrack(track) {
        track.sequences.forEach(sequence => {
            sequence.activeNodes.forEach(({ oscillator, gainNode }) => {
                try {
                    oscillator.stop();
                } catch (error) {
    ...
    playTrack(track) {
        try {
            ...
            track.sequences.forEach(sequence => {
                const sequenceStartTime = sequence.startTime * 1000; 
            ...
            throw new TypeError('Failed to play track:', error.message)
        }
    }
     ...
     */
    addSequenceToTrack(trackIndex, noteObjects, options) {
